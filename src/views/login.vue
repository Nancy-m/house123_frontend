<template>
    <div class="login-wrap">
      <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
        ...
        <el-form-item prop="username">
          <el-input type="text" v-model="ruleForm.email" auto-complete="off" placeholder="電子信箱"></el-input>
        </el-form-item>
        <el-form-item prop="password">
          <el-input type="password" v-model="ruleForm.password" auto-complete="off" placeholder="密碼" show-password></el-input>
        </el-form-item>
        <el-checkbox class="remember" v-model="rememberpwd" @change="change">記住我的信箱</el-checkbox>
        <el-form-item style="width:100%;">
          <el-button type="primary" style="width:100%;" @click="submitForm('ruleForm')" :loading="logining">立即登入</el-button>
        </el-form-item>
        <el-divider>或</el-divider>
        <div
      id="g_id_onload"
      ref="g_id_onload"
      data-client_id="840225314779-v9ghpc2jajgfu15nqh95sqjrjgbunv6u.apps.googleusercontent.com"
      data-context="signin"
      data-ux_mode="popup"
    ...
    ></div>
        <!-- <div class="goo"><img class="google" src="../assets/img/google.png" alt=""/></div> -->
        
      </el-form>
    </div>
  </template>
<script >
import jwt_decode from "jwt-decode"; // 使用jwt decode解码google返回的jwt， 需要安装jwt-decode包
...
//   import md5 from 'js-md5'
  export default {
    ...
    created() {
        // this.init(); //动态插入script标签，引入Google登录
      // 获取存在本地的用户名密码
      this.getuserpwd()
    ...
    methods: {
      ...
      onSignIn(googleUser){
          ...
          console.log('ID: ' + profile.sub) // ID Do not send to your backend! Use an ID token instead.
          console.log('Name: ' + profile.name) //姓名
          console.log('Image URL: ' + profile.picture) // 头像
          console.log('Email: ' + profile.email) // email地址 This is null if the 'email' scope is not present.
      }, 
      // 获取本地用户名密码
      getuserpwd() {
        let em = JSON.parse(localStorage.getItem('email'));
        let pwd = JSON.parse(localStorage.getItem('pwd'));
        if( em && pwd && localStorage.getItem('logintoken')){
          this.ruleForm.email = em;
          this.ruleForm.password = pwd;
          this.rememberpwd = JSON.parse(localStorage.getItem('remember'));
        }else{
          this.ruleForm = { //email和password默认为空
            email: '',
            password: '',
          }
        }
          ...
          if (valid) {
            ...
            login(this.ruleForm).then(res => {
              if (res.code == 200) {
                // if (this.rememberpwd) {
                //   //保存帐号到cookie，有效期7天
                //   setCookie('user', this.ruleForm.email, 7)
                //   //保存密码到cookie，有效期7天
                ...
                setTimeout(() => {
                  ...
                  // 缓存用户个人信息
                  localStorage.setItem('email', JSON.stringify(this.ruleForm.email))
                  localStorage.setItem('pwd', JSON.stringify(this.ruleForm.password))
                  localStorage.setItem('remember', JSON.stringify(this.rememberpwd))
                  res.result.superAdmin == true ? 
                    localStorage.setItem('superAdmin', 'Y'):
                    localStorage.setItem('superAdmin', 'N');
                  // this.$store.commit('login', 'true')
