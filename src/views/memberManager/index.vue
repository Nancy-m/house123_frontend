...
<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>會員資料管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!--查询条件表单组-->
    <div class="search-box">
      <el-form :inline="true" :model="form" ref="selectForm" class="el-form-inline" size="mini" label-position="left" >
        ...
        <el-row>
          ...
          <el-form-item label="" prop="paid">
              <el-select v-model="form.paid" clearable placeholder="请选择是否付費" style="width:100%">
    ...
    <el-table border class="personTable" size="small" highlight-current-row 
      ...
      <el-table-column label="性別" sortable align="center" prop="gender" />
      <el-table-column label="地址" sortable align="center" prop="address" />
      <el-table-column label="是否付費" sortable align="center" prop="paid" width="120">
        <template slot-scope="scope">
          <span v-if="scope.row.paid==true">是</span>
          <span v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column label="付費方式" sortable align="center" prop="pay_method" width="120"/>
      <el-table-column label="創建時間" sortable align="center" prop="create_time" width="120"/>
      <el-table-column label="過期時間" sortable align="center" prop="expire_at" width="120"/>
      <el-table-column label="上次更新時間" sortable align="center" prop="timestamp" width="120"/>
      <el-table-column label="追蹤數據列表" sortable align="center" prop="mark_list" />
      <el-table-column label="付費縣市" sortable align="center" prop="paid_cities" />
...
<script>
import {findUsers,deleteManagers} from '@/api/memberManager'
...
export default {
  ...
  data() {
    return {
      ...
      editForm: {
        ...
        password: "",
        paid: false,
        expire_at: moment().subtract(1, 'day').format('YYYY-MM-DD'),
        paid_cities:[],
        pay_method:'',
      },
  ...
  methods: {
    ...
    async findManagersList(){
      ...
      const res = await findUsers(this.form);
      if(res.code == 200){
        this.loading = false;
    ...
    formInit(){
      this.editForm = {
        ...
        password: "",
        paid: false,
        expire_at: moment().subtract(1, 'day').format('YYYY-MM-DD'), // 默认昨天
        paid_cities:[],
        pay_method:'',
      }
    },
